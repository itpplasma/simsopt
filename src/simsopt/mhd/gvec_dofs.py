...
class GVECSurfaceDoFs(Optimizable):
    ...
    def fixed_range(self, mmin, mmax, nmin, nmax, fixed=True):
        ...
        if mmin > mmax:
            raise ValueError("mmin must not exceed mmax.")
        if mmax > self.mpol:
            raise ValueError("mmax exceeds maximum poloidal mode number. Change resolution first.")
        if nmin < -self.ntor:
            raise ValueError("nmin exceeds maximum toroidal mode number. Change resolution first.")
        if nmin > nmax:
            raise ValueError("nmin must not exceed nmax.")
        if nmax > self.ntor:
