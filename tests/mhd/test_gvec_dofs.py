...
class GVECSurfaceDoFsTests(unittest.TestCase):
    ...
    def test_fixed_range_includes_constant_mode(self):
        surf = GVECSurfaceDoFs(mpol=2, ntor=1, nfp=3)
        surf.unfix_all()

        surf.fixed_range(mmin=0, mmax=0, nmin=0, nmax=0, fixed=True)

        self.assertTrue(surf.is_fixed("X1c(0,0)"))

    def test_fixed_range_rejects_reversed_and_out_of_bounds_ranges(self):
        surf = GVECSurfaceDoFs(mpol=2, ntor=1, nfp=3)

        with self.assertRaisesRegex(ValueError, "mmin must not exceed mmax"):
            surf.fixed_range(mmin=2, mmax=1, nmin=0, nmax=1)
        with self.assertRaisesRegex(ValueError, "nmin must not exceed nmax"):
            surf.fixed_range(mmin=0, mmax=1, nmin=1, nmax=0)
        with self.assertRaisesRegex(ValueError, "nmin exceeds maximum"):
            surf.fixed_range(mmin=0, mmax=1, nmin=-2, nmax=0)
